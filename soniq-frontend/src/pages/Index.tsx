...
const Index = () => {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-background">
      {/* Idle ambient visualizer behind everything */}
      <div className="fixed inset-0 opacity-50 pointer-events-none">
        <VisualizerCanvas idle />
      </div>
      {/* Subtle vignette to keep text readable */}
      <div className="fixed inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse at center, rgba(0,0,0,0.2) 0%, rgba(0,0,0,0.85) 70%, #000 100%)' }} />
      ...
      <section className="relative z-10 min-h-screen flex flex-col items-center justify-center px-6 text-center">
        <h1 className="font-display text-6xl sm:text-7xl md:text-9xl leading-[0.9] text-white text-glow-white">
          FEEL EVERY
          <br />
          <span className="bg-clip-text text-transparent" style={{ backgroundImage: 'linear-gradient(90deg, #FF2D55, #BF5FFF, #00F5FF, #00FFC8, #FFD700)' }}>
            INSTRUMENT
          </span>
        </h1>
        ...
        <div className="mt-10 flex flex-col sm:flex-row gap-3 sm:gap-4">
          <Link
            to="/auth/spotify"
            className="group inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-full border-2 font-mono-ui text-xs tracking-wider transition hover:bg-keys/10"
