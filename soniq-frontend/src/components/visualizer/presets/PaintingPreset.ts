...
// Module-level state — reset via clearPainting() on preset switch or track change
let lastBass = 0, lastDrums = 0;
...
export function renderPainting(
  ...
  // On first frame after a clear, fill black
  if (!canvasCleared) {
    ctx.fillStyle = '#000';
  ...
  // Slow fade — accumulate paint over time
  ctx.fillStyle = 'rgba(0,0,0,0.004)';
  ctx.fillRect(0, 0, w, h);

  // Bass splatter on transient
  if (e.bass - lastBass > 0.2 && e.bass > 0.5) {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      ctx.beginPath();
      ctx.arc(x + Math.cos(a) * d, y + Math.sin(a) * d, 3 + Math.random() * 10 * e.bass, 0, Math.PI * 2);
      ctx.fill();
    }
  }
  lastBass = e.bass;

  // Guitar lightning strokes
  ...
  if (smoothed.vocals > 0.15) {
    ...
    if (vocalPath.length > 1) {
      ...
      ctx.beginPath();
      ctx.moveTo(vocalPath[0].x, vocalPath[0].y);
